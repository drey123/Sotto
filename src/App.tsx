...
export default function App() {
  ...
  const stateRef = useRef({ whispers: SEEDS.map((x) => ({ ...x })), selected: null as number | null });
  ...
  useEffect(() => { stateRef.current.whispers = whispers.map((x) => ({ ...x })); }, [whispers]);
  ...
  useEffect(() => {
    ...
    const floor = new THREE.Mesh(new THREE.CircleGeometry(24, 96), new THREE.MeshStandardMaterial({ color: 0x20282f, metalness: 0.42, roughness: 0.78 }));
    ...
    scene.add(grid);
    const horizonRing = new THREE.Mesh(new THREE.RingGeometry(12.5, 12.58, 128), new THREE.MeshBasicMaterial({ color: 0x6b8da8, transparent: true, opacity: 0.16, side: THREE.DoubleSide }));
    horizonRing.rotation.x = -Math.PI / 2;
    horizonRing.position.y = -1.48;
    scene.add(horizonRing);

    const glowTexture = makeGlowTexture();
    const glowMaterial = new THREE.SpriteMaterial({ map: glowTexture, transparent: true, depthWrite: false, opacity: 0.65 });
    ...
    stateRef.current.whispers.forEach((whisper, index) => {
    ...
    const animate = () => {
      ...
      meshes.forEach((mesh, index) => {
        ...
        const selectedCard = stateRef.current.selected === whisper.id;
        if (selectedCard) {
        ...
        const front = mat[4];
        if (front) {
          front.emissive = new THREE.Color(selectedCard ? 0x24415b : hovered === mesh ? 0x122333 : 0x000000);
          front.emissiveIntensity = selectedCard ? 0.65 : hovered === mesh ? 0.25 : 0;
        }
      });
      raycaster.setFromCamera(pointer, camera);
      const hit = raycaster.intersectObjects(meshes, false)[0]?.object as THREE.Mesh | undefined;
      ...
      glowMaterial.opacity = 0.52 + Math.sin(t * 0.25) * 0.08;
  ...
  const vote = (direction: 1 | -1) => {
    ...
    setSelected((item) => item ? { ...item, votes: Math.max(0, item.votes + direction) } : item);
    if (direction === 1) setStats((s) => ({ ...s, liked: s.liked + 1 }));
  };
  const reveal = () => { if (selected) setUnlocked((items) => items.includes(selected.id) ? items : [...items, selected.id]); };
  const submitWhisper = () => {
    const text = newText.trim();
    if (!text) return;
    const nextId = Math.max(...whispers.map((item) => item.id), 0) + 1;
    const added: Whisper = { id: nextId, text, locked: false, votes: 0, orbit: 0, angle: Math.random() * Math.PI * 2, radius: 4.2, height: -0.4 + Math.random() * 0.8, phase: Math.random() * Math.PI * 2 };
  ...
  return (
    <main className="world-lab">
      ...
      <header className="hud hud-top"><div className="brand">SOTTO</div><div className="hud-caption">A FIELD OF WHISPERS</div></header>
      <div className="hud hud-stats"><span>{stats.discovered} DISCOVERED</span><span>{stats.liked} LIKED</span></div>
      <div className="hud hud-bottom"><button type="button" onClick={() => setComposerOpen(true)}>LEAVE A WHISPER</button><span>DRAG TO MOVE · MOVE CLOSER · SELECT</span></div>
      ...
      {selected && (
        <section className="reading-layer" aria-label="Whisper reader">
          ...
          <article className="reading-object">
            <div className="reading-topline"><span>WHISPER / {String(selected.id).padStart(3, '0')}</span><button type="button" onClick={closeReading}>CLOSE</button></div>
            <div className="reading-content">
              <p className={selected.locked && !unlockedSet.has(selected.id) ? 'is-locked' : ''}>{selected.locked && !unlockedSet.has(selected.id) ? `${selected.text.split(' ').slice(0, 10).join(' ')} …` : selected.text}</p>
              {selected.locked && !unlockedSet.has(selected.id) && <button className="reveal-button" type="button" onClick={reveal}>REVEAL WHISPER · $1 TEST GATE</button>}
            </div>
            <footer className="reading-footer"><div className="vote-row"><button type="button" onClick={() => vote(1)}>↑ {selected.votes}</button><button type="button" onClick={() => vote(-1)}>↓</button></div><span>ANONYMOUS · NO PROFILE</span></footer>
          </article>
        </section>
      )}
